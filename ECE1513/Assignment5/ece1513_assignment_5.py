# -*- coding: utf-8 -*-
"""ECE1513 Assignment 5

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/15JfqSjDwsAc4MWoQG7MweAdEp43QaTRB
...
def loss(params, batch):
  ...
  #return - np.dot(np.divide(1,n),np.sum(np.sum(np.dot(batch, preds)))) #logsumexp with z
...
init_random_params, predict = stax.serial(
    ...
    stax.Conv(64,(8,8),padding = 'SAME', strides = (2,2)),
    stax.Relu,
...
**At here, start for the assignment 5 formal coding:**
"""
...
## From here is Part 1 of assignment
"""
...
input_image, input_label = shape_as_image(test_images[index7], test_labels[index7])
...
labels = np.array(test_labels[0:1000])
input_image, input_label = shape_as_image(images, labels)
hyper_list = np.arange(0,1.1,0.01)
accuracy_list = []
#record the list
for hyper in hyper_list: 
  ...
  grad_newx = grad(computation,1)(params, input_image, input_label)
  newx = input_image + hyper * np.sign(grad_newx)
  #recall the accuracy function 
  target_class = np.argmax(input_label,axis=1)
  predicted_class = np.argmax(predict(params, newx),axis=1)
  acc = np.mean(predicted_class == target_class)
  accuracy_list.append(acc)
#plot
plt.figure 
plt.plot(hyper_list, accuracy_list)
...
#computing process to the new x
grad_newx = grad(computation,1)(params, input_image, input_label)
newx = input_image + epsilon/k * np.sign(grad_newx)

for i in range(k):
  #input_image, input_label = shape_as_image(test_images[index7], test_labels[index7])
  grad_newx = grad(computation,1)(params, newx, input_label)

  newx = newx + (epsilon/k) * np.sign(grad_newx)
target_class = np.argmax(input_label)
predicted_class = np.argmax(predict(params, newx))
#predicted vector
predict_vector = predict(params, newx)
original_verctor = predict(params, input_image)
print('the target class is :%d, the predict class is %d' %(target_class, predicted_class) )
print('the predicted vector is :' ,predict_vector)
print('the orginal vector is :', original_verctor)
...
#record the list
for epsilon in hyper_list: 
  grad_newx = grad(computation,1)(params, input_image, input_label)
  newx = input_image + (epsilon/k) * np.sign(grad_newx)
  #recall the loss function but with k iterations
  for i in range(k):
    #input_image, input_label = shape_as_image(test_images[index7], test_labels[index7])
    grad_newx = grad(computation,1)(params, newx, input_label)
    newx = newx + epsilon/k * np.sign(grad_newx)
